...
| `raw_argument_collections` | ModSecurity PR #3564 | ModSecurity public PR | collections | partial | mapped-only/unsupported-local-source | PR #3564 adds RAW URL-encoded collections, but the current local `ModSecurity_V3` checkout has no RAW collection implementation/regression file | raw args, query args, request body | Add active YAML only after a configured v3 source supports RAW variables and Apache/NGINX both PASS |
