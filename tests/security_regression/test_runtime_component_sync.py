...
class RuntimeComponentSyncTests(unittest.TestCase):
    def run_tool(self, mode: str, *, common=COMMON, manifest=MANIFEST, lock=LOCK):
        return subprocess.run(
            [sys.executable, str(TOOL), mode, "--common-sh", str(common), "--manifest", str(manifest), "--lock", str(lock)],
    ...
    def test_checked_in_inventory_is_current_and_includes_lighttpd(self):
        ...
        self.assertEqual(set(profiles), {
            "nginx-h1", "haproxy-htx", "haproxy-spoe-spop", "envoy-ext-authz",
            "envoy-ext-proc", "traefik-forwardauth", "traefik-native", "lighttpd-sidecar",
        })
        self.assertIn("lighttpd-sidecar", profiles)
        self.assertNotEqual(profiles["haproxy-htx"]["version"], profiles["haproxy-spoe-spop"]["version"])
        self.assertNotEqual(profiles["haproxy-htx"]["sha256"], profiles["haproxy-spoe-spop"]["sha256"])
        self.assertNotEqual(profiles["haproxy-htx"]["download_url"], profiles["haproxy-spoe-spop"]["download_url"])
        self.assertEqual(profiles["haproxy-htx"]["source_url"], profiles["haproxy-htx"]["download_url"])
        self.assertEqual(profiles["lighttpd-sidecar"]["asset_name"], "lighttpd-1.4.85.tar.xz")

        manifest = self.load(MANIFEST)
        lighttpd = next(item for item in manifest["components"] if item["name"] == "lighttpd")
        self.assertTrue(
            lighttpd["sha256_url"].endswith(f"lighttpd-{lighttpd['version']}.sha256sum")
        )
        self.assertEqual(lighttpd["source_stage_dir"], "$BUILD_ROOT/lighttpd-connector/src/lighttpd-1.4.85")

    def test_write_is_idempotent(self):
        temporary, common, manifest, lock = self.fixture()
        with temporary:
            document = self.load(manifest)
            lighttpd = next(item for item in document["components"] if item["name"] == "lighttpd")
            lighttpd["operator_note"] = "fixture metadata must survive synchronization"
            lighttpd["version"] = "1.4.84"
            self.dump(manifest, document)
            result = self.run_tool("--write", common=common, manifest=manifest, lock=lock)
            self.assertEqual(result.returncode, 0, result.stderr)
            synchronized = self.load(manifest)
            synchronized_lighttpd = next(item for item in synchronized["components"] if item["name"] == "lighttpd")
            self.assertEqual(synchronized_lighttpd["operator_note"], "fixture metadata must survive synchronization")
            self.assertEqual(synchronized_lighttpd["version"], "1.4.85")
            self.assertEqual(self.run_tool("--check", common=common, manifest=manifest, lock=lock).returncode, 0)
            manifest_bytes = manifest.read_bytes()
            lock_bytes = lock.read_bytes()
            second = self.run_tool("--write", common=common, manifest=manifest, lock=lock)
    ...
    def test_platforms_are_rendered_from_canonical_artifact_platform_variables(self):
        ...
        with temporary:
            ...
            self.assertTrue(all(item["os"] == "linux" for item in rendered_lock["profiles"]))
            self.assertTrue(all(item["arch"] == "arm64" for item in rendered_lock["profiles"]))
            ...
            self.assertEqual(platforms, {"envoy": "linux/arm64", "traefik": "linux/arm64"})
    ...
    def test_lock_manifest_digest_platform_and_asset_mutations_are_rejected(self):
        lock_mutations = {
            ...
            "asset": lambda d: d["profiles"][0].__setitem__("asset_name", "wrong.tar.gz"),
        ...
        manifest_mutations = {
            ...
            "download": lambda i: i.__setitem__("download_url", "https://example.invalid/lighttpd.tar.xz"),
    ...
    def test_missing_manifest_component_reports_coverage_error(self):
        ...
        with temporary:
            document = self.load(manifest)
            document["components"] = [item for item in document["components"] if item["name"] != "envoy"]
            self.dump(manifest, document)
            result = self.run_tool("--check", common=common, manifest=manifest, lock=lock)
            self.assertEqual(result.returncode, 2)
