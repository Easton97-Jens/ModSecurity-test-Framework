...
SPEC = importlib.util.spec_from_file_location("resolve_canonical_maintenance_cli", CLI_PATH)
...
class FakeOrchestrator:
    ...
    def build_plan(root: Path, *, components: tuple[str, ...], timeout: float) -> dict:
        ...
        components_checked = [
            {"scope": scope, "component_name": scope, "current": "v1.0.0",
             "latest_compatible": "v1.0.0", "latest_upstream": "v1.0.0",
             "status": "current", "updates": []}
            for scope in sorted(MANDATORY_SCOPES)
        ]
        components_checked.append(
            {"scope": "runtime", "component_name": "lighttpd", "current": "1.0.0",
             "latest_compatible": "1.1.0", "latest_upstream": "1.1.0",
             "status": "outdated", "updates": [{"variable": "LIGHTTPD_VERSION", "old": "1.0.0", "new": "1.1.0"}]}
        )
        return {
            "maintenance_outcome": "safe_updates_with_manual_review",
            "safe_updates": [{"variable": "LIGHTTPD_VERSION", "old": "1.0.0", "new": "1.1.0"}],
            "manual_reviews": [{"review_key": "go-ftw:major_version_transition:v3", "reason": "review Go-FTW"}],
    ...
    def render_plan_markdown(plan: dict) -> str:
        return "\n".join(
            ["# Canonical maintenance plan", "", "## Automatic updates", "", "- `LIGHTTPD_VERSION`", "", "## Manual reviews", "", "- `go-ftw:major_version_transition:v3`"]
        ) + "\n"

    @staticmethod
    def apply_safe_updates(root: Path, plan: dict, *, expected_plan_sha256: str) -> list[str]:
...
class ResolveCanonicalMaintenanceCliTests(unittest.TestCase):
    ...
    def test_component_plan_retains_all_mandatory_global_scopes(self) -> None:
        ...
        args = CLI.parse_args(["--root", str(root), "--component", "lighttpd", "--plan", str(plan_path)])
        with mock.patch.object(CLI, "_load_orchestrator", return_value=FakeOrchestrator):
    ...
    def test_manual_review_does_not_hide_safe_runtime_update(self) -> None:
        root = _repo_root()
        output = root / "plan.md"
        args = CLI.parse_args(["--root", str(root), "--component", "lighttpd", "--markdown", str(output)])
        with mock.patch.object(CLI, "_load_orchestrator", return_value=FakeOrchestrator):
            self.assertEqual(CLI.run(args), 0)
    ...
    def test_apply_requires_digest_and_has_no_issue_mutation_surface(self) -> None:
        ...
        with self.assertRaises(SystemExit):
            CLI.parse_args(["--root", str(root), "--plan", str(plan_path), "--apply-safe-updates"])
        args = CLI.parse_args([
            "--root", str(root), "--plan", str(plan_path), "--apply-safe-updates", "--expected-plan-sha256", "a" * 64,
        ])
        with mock.patch.object(CLI, "_load_orchestrator", return_value=FakeOrchestrator):
            self.assertEqual(CLI.run(args), 0)
