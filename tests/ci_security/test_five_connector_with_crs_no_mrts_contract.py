...
FIFO_DIAGNOSTIC_ATTEMPTS = 3
FIFO_DIAGNOSTIC_READY_TIMEOUT_SECONDS = 2.0
FIFO_DIAGNOSTIC_SINK_TIMEOUT_SECONDS = 1.0
FIFO_DIAGNOSTIC_JOIN_TIMEOUT_SECONDS = 1.0


def _fifo_preopen_diagnostic_child(
    ...
    def replace_after_path_validation(value: object, root: Path, label: str) -> Path:
        ...
        if candidate == evidence_path:
            if replaced:
                raise AssertionError("diagnostic evidence leaf was replaced twice")
    ...
    try:
        ...
        path, content, digest = contract._bounded_evidence_file(
            evidence_root,
            artifact,
            "FIFO pre-open diagnostic evidence",
            expected_relative,
...
class FiveConnectorWithCrsNoMrtsContractTest(unittest.TestCase):
    ...
    def _fifo_preopen_diagnostic_attempt(self, attempt: int) -> tuple[bool, str]:
        """Run one bounded child reproduction attempt and always clean it up."""
        with tempfile.TemporaryDirectory(prefix="five-crs-fifo-preopen-") as temporary:
            ...
            artifact = self._artifact(
                root, expected_relative, "record=fifo-pre-open-diagnostic\n"
            )
            ...
            process = context.Process(
                target=_fifo_preopen_diagnostic_child,
                args=(
                    str(root),
            ...
            try:
                ...
                result_sender.close()
                ready_deadline = (
                    time.monotonic() + FIFO_DIAGNOSTIC_READY_TIMEOUT_SECONDS
                )
                ...
                if open_messages:
                    sink_deadline = (
                        time.monotonic() + FIFO_DIAGNOSTIC_SINK_TIMEOUT_SECONDS
                    )
                    remaining = max(0.0, sink_deadline - time.monotonic())
                    if result_receiver.poll(remaining):
                        ...
                        timed_out = True
                        alive_before_cleanup = process.is_alive()
                else:
                ...
                if started and process.is_alive():
                    process.terminate()
                if started:
                    process.join(FIFO_DIAGNOSTIC_JOIN_TIMEOUT_SECONDS)
                    if process.is_alive():
                        process.kill()
                        process.join(FIFO_DIAGNOSTIC_JOIN_TIMEOUT_SECONDS)
            ...
            reproduced = (
                ...
                and not uses_nonblocking
                and timed_out
                and alive_before_cleanup
            ...
            details = (
                ...
                f"timed_out={timed_out}; alive_before_cleanup={alive_before_cleanup}; "
                ...
                f"nonblocking={uses_nonblocking}; pid={process_pid!r}; "
                f"exitcode={process.exitcode!r}"
            )
            if alive_after_cleanup:
                raise AssertionError(
                    "bounded FIFO diagnostic cleanup left its own child alive: "
    ...
    def test_diagnostic_fifo_preopen_hang_is_bounded_and_reproduced(self) -> None:
        outcomes = [
            self._fifo_preopen_diagnostic_attempt(attempt)
            for attempt in range(1, FIFO_DIAGNOSTIC_ATTEMPTS + 1)
        ]
        details = "\n".join(detail for _, detail in outcomes)
        if not all(reproduced for reproduced, _ in outcomes):
            self.fail(
                "bounded FIFO pre-open diagnostic did not establish the expected "
                f"writerless-open timeout:\n{details}"
            )
        self.fail(
            "FIFO pre-open startup hang reproduced in "
            f"{FIFO_DIAGNOSTIC_ATTEMPTS}/{FIFO_DIAGNOSTIC_ATTEMPTS} bounded "
            "writerless-FIFO attempts; no child process or FIFO was retained:\n"
            f"{details}"
        )
