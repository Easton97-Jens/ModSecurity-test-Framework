...
`<workspace>` und `/src` sind nur lokale Standardeinstellungen. Jeder relevante Pfad ist
überschreibbar:
...
- `BUILD_ROOT`: Beschreibbares Stammverzeichnis für Smoke-Objektdateien, Binärdateien und Caches.
- `LOG_DIR`: beschreibbares Verzeichnis für Hilfsprotokolle.

Kein Build-Schritt darf generierte Dateien in `<workspace>/*` oder einen anderen schreiben
Quellenkauf.
...
- Build-Kopie: `/src/ModSecurity_V3_build`
- gebaute Bibliothek: `/src/ModSecurity_V3_build/src/.libs/libmodsecurity.so`
- Hilfsprotokolle:
  - `/src/ModSecurity-test-Framework-build/logs/copy-source.log`
  - `/src/ModSecurity-test-Framework-build/logs/git-submodule-update.log`
  - `/src/ModSecurity-test-Framework-build/logs/build-sh.log`
  - `/src/ModSecurity-test-Framework-build/logs/configure.log`
  - `/src/ModSecurity-test-Framework-build/logs/make.log`
