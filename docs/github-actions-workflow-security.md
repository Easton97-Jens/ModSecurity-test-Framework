...
| `five-connectors-with-crs-no-mrts-contract.yml` | filtered `push`, filtered `pull_request` | `actions/checkout`, `actions/setup-python` | `contents: read` | PR source is untrusted; this read-only gate validates only the portable closed fixture, CRS-provenance, and evidence contract, never a connector-host runtime result. |
