...
Normale Pull-Request-Workflows verwenden `pull_request`. Die einzige eng
begrenzte Ausnahme ist `ci-security-osv.yml`: Sein `pull_request_target`-Job
erhält ausschließlich `contents: read`, checkt die unveränderliche PR-Basis-
SHA ohne persistierte Credentials oder Submodule aus, holt nur
die nummerierte GitHub-Pull-Request-Head-Referenz, verifiziert die gemeldete
Head-SHA und liest die
zwei benannten Dependency-Manifeste als begrenzte Daten. Er checkt den PR-Head
nie aus und führt ihn nicht aus. Alle übrigen Routine-Jobs erhalten nur
`contents: read`, verwenden unveränderliche Action-SHAs mit überprüften
...
ausgecheckte SHA vor dem Scan. Die OSV-target-Ausnahme führt dagegen nur die
