...
`security-events: write` is limited to CodeQL. OSV and Scorecard use
checksum-verified release CLIs outside the checkout, receive no GitHub token,
and do not publish SARIF. Their PR jobs can therefore analyze fork heads with
only `contents: read`. OSV retains only validated, fixed-path JSON comparison
evidence; Scorecard PR jobs remain artifact-free. No workflow uses `id-token:
write`.
