...
Ordinary pull-request workflows use `pull_request`. The sole constrained
exception is `ci-security-osv.yml`: its `pull_request_target` job receives
only `contents: read`, checks out the immutable pull-request base SHA with no
persisted credential or submodules, fetches only the numbered GitHub
pull-request head reference,
verifies the reported head SHA, and reads the two named dependency manifests
as bounded data. It never checks out or executes the pull-request head. All
other routine jobs receive only `contents: read`, use immutable Action SHAs
...
scanning. The OSV target-triggered exception instead runs only the trusted
base revision and uses the verified PR object solely for blob reads. The local
