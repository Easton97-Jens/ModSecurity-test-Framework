# HAProxy SPOE/SPOA Artefakterkennung (nur Dokumentation)

**Sprache:** [English](haproxy-spoe-artifact-discovery.md) | Deutsch
...
artefakt_discovery_implemented: falsch
...
Status aller Felder: nur geplant.

## Vorgeschlagene Discovery-Ausgaben (nur geplant)
...
| Eingabe | Nachweisquelle | Status |
|---|---|---|
| `artifact_root` | Im Connector-Repository zu belegen. | nur geplant |
| `haproxy_config_path` | Im Connector-Repository zu belegen. | nur geplant |
| `spoa_entrypoint_path` | Im Connector-Repository zu belegen. | nur geplant |
| `artifact_manifest_path` | Im Connector-Repository zu belegen. | nur geplant |
| Port-/Netzwerkannahmen | Extern zu verifizieren. | nur geplant |

## Nicht-Ziele
- Keine Validierung durch ausführbares JSON-Schema.
- Anpassung Keine von CI/Build/Runner.
