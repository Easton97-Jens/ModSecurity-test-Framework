...
| `NGINX_BIN`, `NGINX_GITHUB_REPO`, `NGINX_RELEASE_TAG`, `NGINX_SOURCE_MODE`, `NGINX_SOURCE_REPO_URL`, `NGINX_SHA256` | NGINX executable, GitHub URL, release tag, source-mode, or SHA-256 digest override | central pin or target default; `NGINX_SHA256` has no repository default | For the supported `github-release` source mode, set `NGINX_SHA256=<reviewed-64-hex-digest>` for the exact selected archive. Provisioning blocks empty, whitespace-containing, malformed, and mismatching values before latest lookup, cache use, download, or extraction; review the digest again when the tag or source changes. |
