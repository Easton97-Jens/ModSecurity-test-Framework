...
| `NGINX_BIN`, `NGINX_GITHUB_REPO`, `NGINX_RELEASE_TAG`, `NGINX_SOURCE_MODE`, `NGINX_SOURCE_REPO_URL`, `NGINX_SHA256` | NGINX-Programm-, GitHub-URL-, Release-Tag-, Source-Mode- oder SHA-256-Digest-Überschreibung | zentraler Pin oder Target-Standard; `NGINX_SHA256` hat keinen Repository-Standard | Für den unterstützten Source-Mode `github-release` muss `NGINX_SHA256=<reviewed-64-hex-digest>` für das exakt ausgewählte Archiv gesetzt sein. Das Provisioning blockiert leere, Whitespace enthaltende, malformed und abweichende Werte vor Latest-Lookup, Cache-Nutzung, Download oder Extraction; bei Änderungen von Tag oder Source den Digest erneut prüfen. |
