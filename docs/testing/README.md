...
evidence, former expected-failure/mapped-only decisions, and connector-free API smoke boundary.
...
| `response-body-blocking-investigation.md` | Evidence for keeping `RESPONSE_BODY` blocking former expected-failure/mapped-only |
...
| `no-crs-baseline.md` | Canonical capability-driven all-connector No-CRS evidence contract |

## Rule

`pass` means real observed behavior. `former expected-failure` and `mapped-only` cases remain
