...
| `tests/modsecurity-phase4-modes.t` | Strict mode aborts after headers were already sent | `strict` | `GET /x` | `Hello strict` | Test::Nginx expects empty reply and phase4 log has `actual_action=connection_abort` | no | `tests/cases/connector-specific/nginx/nginx_phase4_strict_connection_abort.yaml` | xfail | Current smoke schema asserts stable HTTP status codes; observed curl empty-reply behavior is evidence but not an active PASS | response-body, phase4, intervention, logging | Needs explicit empty-reply/connection-abort assertion before active import |
