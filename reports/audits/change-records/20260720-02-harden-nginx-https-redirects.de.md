...
| Issue oder Pull Request | SonarCloud AZ9_o2_jSLr5VHr-smcj (shell:S6506); die Task-Branch-Auslieferung steht aus und kein Merge ist autorisiert. |
...
HTTPS-Optionswerte verlangte; nach dem Patch bestand sie. Die zusätzlichen
Containment-Kontrollen verwerfen /tmp, ein nicht passendes Snapshot-Ziel und
eine Ausgabe von Case-Informationen außerhalb der Root, während legitime
enthaltene Schreibvorgänge erhalten bleiben.
...
| --- | --- | --- | --- |
| AZ9cRqtu1JCbMyYXCAue | python:S5443 | ci/checks/catalog/no_crs_baseline.py:1746 | /tmp, öffentliche Parents, Symlink-Komponenten und Source-Checkouts werden vor Nutzung einer Run-Root verworfen. |
| AZ7Wh-x6WJ9AQTOMyhFJ | pythonsecurity:S8707 | ci/reporting/update-runtime-snapshot.py:72 | Das Snapshot-Ziel muss dem kanonischen Report-Snapshot unter der gewählten Output-Root entsprechen. |
| AZ5Q3NAAoI4Cm-ZmWjGX | pythonsecurity:S2083 | ci/reporting/update-runtime-snapshot.py:72 | Dieselbe kanonische Snapshot-Zielprüfung gilt. |
| AZ55dzzC6nhd5cS8C48e | pythonsecurity:S2083 | tests/runners/runner_core.py:636 | contained_write_path verwirft eine Ausgabe außerhalb der vertrauenswürdigen Root vor dem Schreiben. |
| AZ6jf1K_DIaptS4_Hf5n | pythonsecurity:S2083 | tests/runners/case_cli.py:424 | case-info verwendet dieselbe Prüfung und eine erforderliche vertrauenswürdige Output-Root. |

Offizielle Flows der letzten vier Zeilen zeigen tainted Inhalt, der eine
Write-API erreicht, nicht die Umgehung von Path-Containment-Kontrollen. Die
Zeilen bleiben im externen Analyzer offen; es wurde keine
False-Positive-, Unterdrückungs- oder Accepted-Issue-Aktion ausgeführt.
...
Der aktuelle Master bleibt rot, weil fünf historische
Framework-Sicherheitszeilen und 17 extern verantwortete MRTS-Metadatenzeilen
offen bleiben. Framework-Kontrollen liefern konkrete Gegenbeweise für das
gemeldete Write-Path-Risiko, können aber keinen externen Analyzer-Status ändern.
Eine Post-Delivery-Master-Verifikation braucht separate
Integrationsautorisierung; dieser Task hat keine Merge-Berechtigung. Der
...
Fokussierte Tests, expliziter-Root-Full-Lint und `git diff --check` bestanden.
Aus stehen finaler Scoped-/Security-Review, normaler Task-Branch-Commit, Push,
Draft-PR und Exact-Head-Remote-Readback. Es ist keine Parent-, Gitlink-,
MRTS-Quell- oder Analyzer-Konfigurationsänderung enthalten.
