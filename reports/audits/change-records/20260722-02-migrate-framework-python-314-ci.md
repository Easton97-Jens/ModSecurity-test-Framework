...
| Issue or pull request | Framework PR #42; the initial CPython-3.14.6 migration was submitted as `e0564d219980d62bc37162ac6c11641f289f1b71`. Hosted CPython 3.14.6 availability and Sonar passed there, while OSV exposed a trusted-base CP313/PR-head CP314 lock mismatch and Ruff reported formatting drift. The security-preserving source follow-up at `2930e04e1558b5b10bdeb87a76abb077a2085566` corrected those controls and the focused Pyright test-fixture typing issue. Its hosted `python-ci-security-quality` run `29962792445` / job `89067507532`, repaired OSV control, all non-skipped PR checks, and PR SonarQube Cloud Quality Gate passed. PR #42 then merged normally at `2026-07-23T07:41:13Z` as Framework master commit `935cf14c676a24672be5c336e92cd13457cc35c8`; its retained PR description correctly identifies `2930e04` as historical evidence and `dc6cf411e78b3f37f1e4be52edef59894560b1ae` as the final PR head. This resulting-master receipt does not classify the independent current-master Sonar finding `FND-SONAR-0002` or the queued Cloudflare suite as passing. Every later PR head still requires fresh exact-head evidence. |
...
| Python.org updater/network validation and resulting-master receipt | `not_run` / observed | No Python.org live updater-network run was performed. The normal PR #42 merge and resulting Framework master commit `935cf14c676a24672be5c336e92cd13457cc35c8` are observed. That receipt does not claim passing current-master Sonar or the queued Cloudflare suite. Every later PR head requires fresh exact-head evidence. | GitHub PR #42 merge receipt |
...
The paired CI-security guide documents the 3.14.6 baseline, strict stable CPython 3.14 grammar, controlled candidate path, Python.org updater, publisher branch, static-tool baselines, exact CP314 PyYAML tuple, and retained OSV trust boundary including its one SHA-bound CP313 compatibility case. The README indexes this pair. Local contract and lock-dry-run evidence was collected with CPython 3.14.4; exact source follow-up `2930e04e1558b5b10bdeb87a76abb077a2085566` additionally passed hosted Python quality, repaired OSV, other non-skipped PR checks, and the PR SonarQube Cloud Quality Gate. PR #42 then merged normally at `935cf14c676a24672be5c336e92cd13457cc35c8`; this resulting-master receipt does not substitute for fresh evidence for a later PR head and does not claim the independent Sonar or queued Cloudflare state passed. No connector runtime, Python.org live updater request, or real package installation was claimed.
...
- No Python.org live updater-network run was added after the merge. The PR #42 resulting-master receipt is observed at `935cf14c676a24672be5c336e92cd13457cc35c8`; this historical record does not claim a result for any later documentation-correction head.

## Limitations and residual risk

Exact source follow-up `2930e04e1558b5b10bdeb87a76abb077a2085566` proves its hosted PR event results, including Python quality, repaired OSV, other non-skipped PR checks, and the PR SonarQube Cloud Quality Gate. The normal PR #42 merge/resulting-master receipt at `935cf14c676a24672be5c336e92cd13457cc35c8` is observed, but it cannot prove the GitHub event context of a later PR head, `runner.temp` semantics, branch protection, or that independent current-master Sonar and queued Cloudflare states pass. The selected local runner is CPython 3.14.4, not the configured 3.14.6. This record makes no connector-runtime claim and does not change the read-only `tools/MRTS` boundary.

## Final diff and review status

The paired documentation passed local link, variable, path-reference, Change Record, whitespace, and scoped-diff checks as part of the native lint contract. The exact source follow-up received a final-diff security review with no reportable High/Critical issue, was committed and pushed normally, and passed its hosted exact-head checks at `2930e04e1558b5b10bdeb87a76abb077a2085566`. PR #42 subsequently merged normally at `935cf14c676a24672be5c336e92cd13457cc35c8`; the independent current-master Sonar and queued Cloudflare limits remain explicitly unresolved. Every later PR head requires new exact-head verification. No credential, token, raw log, or sensitive payload is recorded here.
