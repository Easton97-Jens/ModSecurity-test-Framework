...
if ! "$PYTHON_BIN" "$FRAMEWORK_ROOT/ci/tools/check-python-deps.py"; then blocked "missing Python dependencies for selected interpreter"; fi
...
if detected=$(BUILD_ROOT="$BUILD_ROOT" MODSECURITY_V3_SOURCE_DIR="${MODSECURITY_V3_SOURCE_DIR:-}" FRAMEWORK_ROOT="$FRAMEWORK_ROOT" CONNECTOR_ROOT="$CONNECTOR_ROOT" sh "$FRAMEWORK_ROOT/ci/provisioning/find-modsecurity-v3.sh"); then
