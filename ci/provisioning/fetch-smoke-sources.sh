...
provision_fresh_modsecurity_v3() {
    ...
    if ! ci_provision_approved_modsecurity_v3_checkout "$MODSECURITY_V3_SOURCE_DIR"; then
        cleanup_failed_modsecurity_v3_provision
        notice "ModSecurity v3 provisioning blocked for approved commit $MODSECURITY_V3_APPROVED_COMMIT"
        exit 77
    fi
}
