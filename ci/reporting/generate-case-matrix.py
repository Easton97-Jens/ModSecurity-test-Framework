...
for path in (RUNNER_DIR, LIB_DIR):
...
LEGACY_FLAT_GENERATED_DIR = "docs/testing/generated"
...
def generated_de_filename(name: str) -> str:
    return name.removesuffix(".md") + ".de.md"


def cleanup_legacy_framework_generated_reports() -> None:
    """Remove superseded flat generated reports only as part of regeneration.

    Category directories under ``docs/testing/generated`` are canonical.  The
    former flat copies were stale duplicates, so this source-of-truth cleanup
    intentionally runs only for framework-owned output.
    """
    if OUTPUT_ROOT != FRAMEWORK_ROOT:
        return
    legacy_root = resolve_under_root(
        FRAMEWORK_ROOT,
        FRAMEWORK_ROOT / LEGACY_FLAT_GENERATED_DIR,
        label="legacy generated report root",
    )
    for name in GENERATED_REPORT_NAMES:
        for legacy_name in (name, generated_de_filename(name)):
            path = resolve_under_root(legacy_root, legacy_root / legacy_name, label="legacy generated report")
            if path.is_file():
                path.unlink()


def normalize_localized_overview_report_links() -> None:
    """Keep the tracked German generated overview aligned with canonical paths.
    ...
    if not overview_de.is_file():
        return
    text = overview_de.read_text(encoding="utf-8")
    updated = text
    for name, category in GENERATED_REPORT_CATEGORIES.items():
    ...
    def _write_known(self, path: Path, body: str) -> None:
        ...
        if REPORT_UTILS is not None and path.name in GENERATED_REPORT_NAMES:
            ...
            metadata = REPORT_UTILS.build_metadata(
                generated_by="framework:ci/reporting/generate-case-matrix.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    cleanup_legacy_framework_generated_reports()
    ...
    normalize_localized_overview_report_links()
    return 0
