...
load_nginx_adapter_metadata() {
    eval "$(CONNECTOR_ROOT="$CONNECTOR_ROOT" "$PYTHON_BIN" "$FRAMEWORK_ROOT/ci/lib/adapter_metadata.py" shell nginx --prefix NGINX_ADAPTER)"
}
...
if [ "$needs_build" -eq 1 ]; then
    ...
    if command -v timeout >/dev/null 2>&1; then
        ...
        FRAMEWORK_ROOT="$FRAMEWORK_ROOT" CONNECTOR_ROOT="$CONNECTOR_ROOT" sh "$FRAMEWORK_ROOT/ci/provisioning/prepare-nginx-build.sh"
    else
