...
prepare_crs_variant() {
    ...
    MODSECURITY_RULE_PREAMBLE_FILE=$(combine_rule_preambles "$crs_preamble" "$existing_preamble" "apache-with-crs-${MODSECURITY_MRTS_VARIANT:-no-mrts}")
    export MODSECURITY_RULE_PREAMBLE_FILE
}

load_apache_adapter_metadata() {
    eval "$(CONNECTOR_ROOT="$CONNECTOR_ROOT" "$PYTHON_BIN" "$FRAMEWORK_ROOT/ci/lib/adapter_metadata.py" shell apache --prefix APACHE_ADAPTER)"
}
...
echo "run_apache_smoke: MODSECURITY_MRTS_VARIANT=$MODSECURITY_MRTS_VARIANT"
...
resolve_modsecurity_v3_source_dir() {
    ...
    if detected=$(FRAMEWORK_ROOT="$FRAMEWORK_ROOT" CONNECTOR_ROOT="$CONNECTOR_ROOT" sh "$FRAMEWORK_ROOT/ci/provisioning/find-modsecurity-v3.sh"); then
...
if [ "$needs_build" -eq 1 ]; then
    ...
    if command -v timeout >/dev/null 2>&1; then
        ...
        FRAMEWORK_ROOT="$FRAMEWORK_ROOT" CONNECTOR_ROOT="$CONNECTOR_ROOT" sh "$FRAMEWORK_ROOT/ci/provisioning/prepare-apache-build.sh"
    else
